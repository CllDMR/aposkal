...
export const getCompanyById = async ({ companyId, userId, getSession = false }) => {
...
export const createCompany = async ({ company, userId }) => {
    ...
    if (!validation.success) return { error: validation.error.format(), code: 400 };
...
export const addUserToCompany = async ({ companyId, email, role = "USER", name, phone, invitedBy }) => {
    companyId = parseInt(companyId);
...
export const getInvitation = async ({ inviteId, email }) => {
    ...
    if (!invitationObject.invitedUser) return { error: "User not found", code: 404 };

    invitationObject.isNewUser = invitationObject.invitedUser.changePasswordCode ? true : false;
    ...
    if (!invitationObject.companyUser) return { error: "Invitation not found", code: 404 };

    // check invitationObject.invitedUser.email === email
    if (invitationObject.invitedUser.email !== email) return { error: "Invitation not found", code: 404 };
...
export const acceptInvite = async ({ inviteId, userId, email, password }) => {
    ...
    if (!invitedUser || invitedUser.email !== email) return { error: "User not found", code: 404 };
...
export const updateUserInCompany = async ({ companyId, userId, isActive, role }) => {
    companyId = parseInt(companyId);
    ...
    if (authUserInCompany.role !== "OWNER" && authUserInCompany.role !== "ADMIN") return { error: "User not authorized", code: 403 };
    ...
    if (userInCompany.role === "OWNER") return { error: "3 User not authorized", code: 403 };
    // update user
...
export const removeUserFromCompany = async ({ companyId, userId }) => {
    ...
    // check user role is OWNER owner can not be updated
    if (userInCompany.role === "OWNER") return { error: "User not authorized", code: 403 };
