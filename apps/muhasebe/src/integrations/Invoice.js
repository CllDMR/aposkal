...
const options = {
    ProfileID: ["TEMELFATURA", "TICARIFATURA", "YOLCUBERABERFATURA", "IHRACAT", "KAMU", "HKS", "EARSIVFATURA"],
    InvoiceTypeCode: ["SATIS", "IADE", "TEVKIFAT", "ISTISNA", "IHRACKAYITLI", "OZELMATRAH", "SGK", "KOMISYONCU"],
    DocumentDescription: ["AVANS", "YEMEK_FIS", "E-FATURA", "E-FATURA_IRSALIYE", "E-ARSIV", "E-ARSIV_IRSALIYE", "FATURA", "OTOPARK", "FATURA_TAHSILAT", "FATURA_TAHSILAT_KOMISYONLU"],
}
...
class Invoice {
    ...
    setIssueDate(value) {
        // YYYY-MM-DD
        if (value.length !== 10) throw new Error("IssueDate must be 10 characters long");
        this.IssueDate = value;
    }
    setIssueTime(value) {
        // HH:MM:SS
        if (value.length !== 8) throw new Error("IssueTime must be 8 characters long");
    ...
    setDocumentCurrencyCode(value) {
        if (!currencyCodes.includes(value)) throw new Error("DocumentCurrencyCode must be a valid currency code");
    ...
    setBillingReference_OKC(id, issueDate, documentDescription, externalReference, startDate, startTime, endpointID) {
        checkOptions(options.DocumentDescription, documentDescription, "DocumentDescription setBillingReference_OKC");
