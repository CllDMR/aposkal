...
export async function POST(request) {
    ...
    const companyData = await createCompany({ company: body, userId: checkAtuhRes.user.id });
    if (companyData.error) return NextResponse.json(companyData.error, { status: 400 });
