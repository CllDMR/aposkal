...
export async function POST(request) {
    ...
    const inviteRes = await addUserToCompany({ companyId, email, role: "USER", name, phone, invitedBy });

    if (inviteRes.error) return NextResponse.json(inviteRes.error, { status: 400 });
